...
crit_path <- rprojroot::find_root(has_dir("src")) 
...
```{r load-data}
# Load the dataset ----
world_data  <- read.csv(file = file.path(data_path, "World.csv"))
kenya_data  <- read.csv(file = file.path(data_path, "Kenya.csv"))
sweden_data <- read.csv(file = file.path(data_path, "Sweden.csv"))
```

*The readr::read_csv function was not able to read in these three files correctly in this computer particularly, after discussing with Victor, I used read.csv instead. 
...
The above results show the similar pattern that for all three datasets, as the period moved from 1950-1955 to 2005-2010, the crude birth rate decreased.
...
sweden_data$asfr
```
In general, Kenya has higher ASFRs than Sweden, especially in period 1950-1955. 
...
```
In general, the number of women in 2005-2010 is 2.525256 times bigger than the number of women in 1950-1955. The total number of birth in 2005-2010 is 1.37 times bigger than the total number of birth in 1950-1955.
...
In general, the crude death rate decreases from 1950-1955 to 2005-2010. The magnitude of decrease is larger for Kenya than that for Sweden. 
...
# Compare ASDR for Kenya and Sweden during 2005-2010
kenya_data$asdr[16:30]
sweden_data$asdr[16:30]
```
During the period of 2005-2010, in general, Sweden has lower ASDR rate than Kenya. 
...
```
The counterfactual CDR for Kenya does not change much from 1950-1955 to 2005-2010. This change rates is much smaller than the rate from the original CDR of Kenya, and is very similar to the change rate of Sweden as well. 
