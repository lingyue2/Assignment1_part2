...
SwedenData$py<-SwedenData$py.men+SwedenData$py.women
```
...
(swedencbr <-compute_cbr(SwedenData))
```
...
SwedenData <- compute_asfr(SwedenData)
```
...
(sweden_tfr <- compute_tfr(SwedenData))
```
...
(sweden_cdr <- compute_cdr(SwedenData))
```
...
SwedenData <- compute_asdr(SwedenData)
```
...
SwedenData<- compute_pop_prop(SwedenData)
```

```{r}
# Compute Kenyas CDR Kenya had Sweden's population distribution
mutate(kenya_data,
 temp_cdr  = asdr * SwedenData$popP) %>%
 group_by(period) %>%
